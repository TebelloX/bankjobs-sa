...
- [x] **Investec — DONE 21 Jul 2026 (c53c08a); discovery re-routed 28 Jul 2026.** eArcu adapter live: 5 SA roles at launch. Originally session cookie + pagestamp → `posbrowser_gridhandler` HTML fragment → per-page JSON-LD. **Since ~midday SAST 27 Jul 2026 AWS WAF JS-challenges that results page** — HTTP 202 + `x-amzn-waf-action: challenge` + empty body for any client that can't run the challenge script (browser UA strings included), and 202 being 2xx meant the old `res.ok` check sailed past it and every scheduled run died at pagestamp extraction with a misleading message. Discovery now reads the **unchallenged `/jobs/sitemap.xml`** (`<urlset>`; keep `/jobs/vacancy/…/{id}/description/`, exclude the ~44 evergreen `/jobs/talentpool/…` entries) → same per-page JSON-LD. Every eArcu fetch now demands `status === 200` and names the WAF header in its error. Standing gotchas honoured in code: `datePosted` is templated junk (postedDate stays null, firstSeen carries recency); `validThrough` never used for closure; corporate www.investec.com is Cloudflare-walled — never fetched.
...
- [x] **Postbank (SA Postbank SOC Ltd) — DONE 26 Jul 2026.** Not one of the original five; a state bank with NO ATS at all. `www.postbank.co.za/careers.html` is a hand-maintained HTML table (position → PDF advert, location, closing date) and the PDF advert IS the job ad. Two things make it work: (1) **closed ads are never removed** — the table is a rolling annual archive (64 rows listed, 2 still open at build time), so the closing-date filter (>= today in SAST, keeping a role that closes today; unreadable dates dropped and logged) is the adapter's load-bearing logic and the open count is small and lumpy by nature; (2) PDF text is extracted with `unpdf` through a new shared `pdf/` client that rebuilds READING ORDER from item geometry — PDF.js content-stream order hoists every section heading to the top of its page, which would sever "Minimum Requirements" from the qualifications block core's heading-windowed requirements extractor reads. hrefs are Windows paths with raw spaces/en-dashes/ampersands (normalised and percent-encoded); ids are slugs of the PDF filename (the only stable identifier — re-adverts correctly become new postings); applyUrl is the advert PDF, which names the mailbox to apply to. No posted date exists anywhere, so postedDate is null.
