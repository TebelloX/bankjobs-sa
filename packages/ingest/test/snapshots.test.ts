...
describe('emitSnapshots', () => {
  ...
  it('reports SA vs international counts and rollups in meta.json', () => {
    ...
    // absa, firstrand, standardbank, investec, gotyme, nedbank all present
    // (gotyme is seeded disabled but still a sources row, so it appears with
    // count 0).
    expect(meta.sources.length).toBe(6);
