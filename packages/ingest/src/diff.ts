...
function insertLocations(db: JobsDb, jobId: string, job: CanonicalJob): void {
  for (const row of locationRows(job)) {
    db.run('INSERT INTO job_locations (job_id, city, province) VALUES (?, ?, ?)', [
      jobId,
      row.city,
      row.province,
    ]);
...
/**
 * Insert new jobs and reconcile existing ones. For every seen job we always
 * refresh lifecycle fields (`last_seen`, `missed_runs`, `status`, `closed_at`)
 * WITHOUT touching content columns, so unchanged jobs never churn the FTS index
 * nor bump `updated_at`. Only when the content hash differs do we rewrite the
 * content columns and refresh `job_locations`. Reappearing closed jobs reopen;
 * `first_seen` is preserved. Intended to run inside one caller-owned transaction.
 */
export function upsertJobs(
  ...
  for (const job of jobs) {
    ...
    if (existing === undefined) {
      ...
      insertLocations(db, job.id, job);
      inserted += 1;
      continue;
    }

    // Lifecycle refresh — never mentions title/description_text, so the FTS
    // update trigger (scoped to those columns) does not fire here.
    db.run(
      `UPDATE jobs SET last_seen = ?, missed_runs = 0, status = 'open', closed_at = NULL WHERE id = ?`,
      [now, job.id],
    );
    ...
    if (existing.content_hash !== hash) {
      ...
      db.run('DELETE FROM job_locations WHERE job_id = ?', [job.id]);
      insertLocations(db, job.id, job);
      updated += 1;
    } else {
      unchangedContent += 1;
  ...
  return { seen: jobs.length, inserted, updated, unchangedContent };
}
 ...
 */
export function closeAbsentees(
  db: JobsDb,
  source: SourceId,
  runStartedAt: string,
  now: string,
