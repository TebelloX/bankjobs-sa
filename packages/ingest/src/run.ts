...
export interface RunOptions {
  ...
  dryRun: boolean;
  fixtures: boolean;
  /** The opened DB seam (local or D1); `runIngest` closes it in its finally. */
  db: JobsDb;
  /** Emit the static site snapshots (true locally, false in remote mode). */
  snapshots: boolean;
  snapshotDir: string;
  log: (msg: string) => void;
}
...
export async function runIngest(opts: RunOptions): Promise<number> {
  const { sources, dryRun, fixtures, db, snapshots, snapshotDir, log } = opts;
  ...
  try {
    for (const source of sources) {
      ...
      try {
        ...
        if (!dryRun) {
          ...
          runId = ins.lastInsertRowid ?? undefined;
        }
        ...
        const baseline = await db.get<{ jobs_seen: number | null }>(
        ...
        if (dryRun) {
          const existingIds = new Set(
            (await db.all<{ id: string }>('SELECT id FROM jobs WHERE source = ?', [source])).map(
              (r) => r.id,
            ),
          );
          ...
          const openIds = (
            await db.all<{ id: string }>(
              "SELECT id FROM jobs WHERE source = ? AND status = 'open'",
              [source],
            )
          ).map((r) => r.id);
        ...
        await db.transaction(async () => {
          const r = await upsertJobs(db, source, jobs, runStartedAt);
          inserted = r.inserted;
          updated = r.updated;
          unchanged = r.unchangedContent;
          if (guard.ok) {
            closed = await closeAbsentees(db, source, runStartedAt, runStartedAt);
          } else {
            log(
        ...
        if (outcome === 'success') {
          await db.run('UPDATE sources SET last_success_at = ? WHERE id = ?', [
            runStartedAt,
            source,
          ]);
        } else {
    ...
    if (!dryRun) {
      ...
      // Remote runs skip the site snapshots (the API serves live D1); the
      // staleness backstop above still runs on every non-dry run.
      if (snapshots) {
        await emitSnapshots(db, snapshotDir, nowFinal);
        log(`snapshots written to ${snapshotDir}`);
      } else {
        log('snapshots skipped (remote mode)');
      }
    }
